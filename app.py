...
if st.button("🎯 Get Assessment Recommendations"):
    # Prepare user input
    skill_list = skills
    input_text = f"{job_role} {' '.join(skill_list)} {industry} {experience_level} {job_description_text}"
    ...
    catalogue_df['Similarity Score'] = similarity_scores
    recommendations = catalogue_df.sort_values(by='Similarity Score', ascending=False).head(5).copy()
